...
func testEvaluation() {
	...
	testEvaluationSingle("", "B=[20,{'b':4},'d','e'];C=B.slice(1);C.concat(B,B,B).length", "11", KindANY)
	testEvaluationSingle("", "B=[20,{'b':4},'d','e'];C=B.slice(1);C.concat(B,B,B);C[4]", "null", KindANY)
	testEvaluationSingle("", "B=[20,{'b':4},'d','e'];C=B.slice(1);D=C.concat(B,B,B);D[4]", "20", KindANY)
	testEvaluationSingle("", "B=[20,{'b':4},'d','e'];B.copyWithin(1,0,1);B[1]", "20", KindANY)
	testEvaluationSingle("", "B=[20,5,'d','e'].entries();C=B.next().value();D=B.next().value();C+D", "25", KindANY)
	...
	testEvaluationSingle("", "B=[1,2,[3,4,5,6]].flatMap((val,ind)=>{if (Array.isArray(val)){val[0]=ind}else{val-ind} });B.length*1000+B[1]*100+B[2]*10+B[3]", "6124", KindANY)
	...
	testEvaluationSingle("", "B=[1,2,3,4].map((val,ind)=>{val==ind+1});B[3]", "7", KindANY)
	...
	testEvaluationSingle("", "B=[1,2,3,4];B.reduceRight()", "4", KindANY)
	...
	testEvaluationSingle("", "B=[1,2,3,4];B.some((val,ind)=>{val==3})", "4", KindANY)
	...
	testEvaluationSingle("", "B=[11,22].values();C=B.next().value;D=B.next().value;B.next()", "null", KindANY)
